...
**Transición default entre escenas**: todo `@back` que abre una escena o cambia de ubicación (no los cambios de apariencia de personaje) usa la transición `LineReveal` con `time:2` — por ejemplo `@back "Universidad.LineReveal" time:2`. Usar este mismo valor por defecto en cualquier `@back` nuevo, salvo que se pida explícitamente otro efecto (ver lista completa de transiciones disponibles en `Naninovel.TransitionType`, `Packages/com.elringus.naninovel/Runtime/Transition/TransitionType.cs`). **Regla general (por pedido del usuario, el juego se sentía muy acelerado): ninguna transición — `@back`, movimiento de personaje (`pos:` con `time:`), etc. — debería durar menos de 2 segundos.** La Escena 1 no usa `LineReveal` (excepción ya documentada más abajo en este archivo si aplica), pero igual lleva `time:2` para respetar esta duración mínima.
...
**Animación de entrada de Hazel/Vaelyr/Egis**: la primera vez que aparece el sprite de cualquiera de estas tres (no las reapariciones posteriores del mismo personaje dentro de la misma escena, ni los cambios de apariencia), en vez de mostrarlas directo en `pos:60,-60` aparecen en `pos:60,60` y se deslizan a su posición final en 2 segundos, con un segundo `@char`:

```
@char Egis.Egis-seria scale:1 pos:60,60
@char Egis pos:60,-60 time:2
```

Después de la animación todo sigue igual que el resto de personajes (mismo `scale:1 pos:60,-60` final). No aplica a Marr/Bibliotecaria (ya están en `pos:60,0`, no tendrían adónde deslizarse) ni a los Extras Genéricos (no lo pidió el usuario). Para Vaelyr en la Escena 2.4, donde comparte pantalla con un Extra y usa X=65 en vez de 60 (para no superponerse), la animación anima igual solo el eje Y (`pos:65,60` → `pos:65,-60`), manteniendo su X propio en ambos puntos.

**BGM al principio de cada escena**: `@bgm` no corta otras pistas por su cuenta (Naninovel permite varias sonando a la vez) — por eso cada escena antepone `@stopBgm` a su propio `@bgm`, **excepto cuando la pista es la misma que ya venía sonando de la escena anterior en la cadena real** (ej. Escena 2.1 y "Días 1 a 3" siguen con el "Tema de campus" que arrancó en Escena 1) — ahí no se pone `@stopBgm`, porque `@bgm` ya no reinicia una pista que ya está sonando (sigue sin cortes). Naninovel no expone ninguna función de expresión para consultar en tiempo real qué BGM está sonando, así que esto se decide de forma estática al escribir cada script, mirando qué pista deja sonando la escena anterior en la cadena — si en algún momento se reordena la cadena de escenas, hay que revisar esto a mano.
